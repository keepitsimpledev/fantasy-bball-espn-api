from src.constants import ALL_STATS, ESPN_STATS_KEY, ESPN_STATS_TOTAL_KEY, KEY_IR, MY_ESPN_S2, MY_SWID
...
def get_league(league_id, year):
    league = League(league_id, year, espn_s2=MY_ESPN_S2, swid=MY_SWID)
...
def format_team_name(team: Team):
    ...
    return remove_unallowed_characters(formatted_name)


# todo: add a comment for this function indicating what this function solves
def remove_unallowed_characters(name: str):
...
def extract_schedules_from_league(league: League):
    ...
    for team in league.teams:
        ...
        for week in team.schedule:
            home_team_name = format_team_name(week.home_team)
            away_team_name = format_team_name(week.away_team)
            opponent = home_team_name if team_name == away_team_name else away_team_name
            schedules[team_name].append(opponent)
    return schedules
