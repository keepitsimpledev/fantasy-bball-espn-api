...
def validate_league_structure(league: League):
    ...
    if len(league.teams) < 1:
        raise EspnClassStructureError("ESPN League's `teams` attribute is, unexpectedly, empty")
...
def validate_teams_structure(teams: list[Team]):
    for team in teams:
        ...
        raise_if_attribute_is_empty("ESPN Team {}'s roster".format(team.team_name), team.roster)
        if not hasattr(team, "team_abbrev"):
            logger.info("ESPN Team {} is missing optional `team_abbrev` attribute".format(team.team_name))
...
def raise_if_attribute_is_empty(object_name: str, object):
    if len(object) < 1:
        raise EspnClassStructureError(
            object_name + " object is unexpectedly empty"
        )
